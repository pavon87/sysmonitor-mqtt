...
script_name = 'sysmonitor.py'
script_info = '{} v{}'.format(script_name, script_version)
project_name = 'Sysmonitor. Fork of RPi Reporter MQTT2HA Daemon'
...
def getCpu():
    ...
    out = subprocess.Popen("mpstat -o JSON |grep number-of-cpus",
           shell=True,
           stdout=subprocess.PIPE,
           stderr=subprocess.STDOUT)
    stdout,_ = out.communicate()
    rpi_cpun_temp = stdout.decode('utf-8').rstrip()
    rpi_cpun = int(rpi_cpun_temp.split(",")[0].split(":")[1].split(" ")[1])

    out = subprocess.Popen("mpstat -o JSON |grep idle |awk '{print $22}'",
           shell=True,
           stdout=subprocess.PIPE,
           stderr=subprocess.STDOUT)
    stdout,_ = out.communicate()
    rpi_cpup_temp = stdout.decode('utf-8').rstrip().split("}")
    rpi_cpup = round((100 - float(rpi_cpup_temp[0])),2)

    #print_line('rpi_cpup=[{}]'.format(rpi_cpup), debug=True)
    #print_line('rpi_cpun=[{}]'.format(rpi_cpun), debug=True)

    rpi_cpu_tuple = ( rpi_cpup, int(rpi_cpun) )
...
uniqID = "Sysmonitor-{}Mon{}".format(mac_left, mac_right)
...
detectorValues = OrderedDict([
    (LD_MONITOR, dict(title="System Monitor {}".format(rpi_hostname), device_class="timestamp", no_title_prefix="yes", json_value="timestamp", json_attr="yes", icon='mdi:raspberry-pi', device_ident="RPi-{}".format(rpi_fqdn))),
    (LD_FS_USED, dict(title="System Used {}".format(rpi_hostname), no_title_prefix="yes", json_value="fs_free_prcnt", unit="%", icon='mdi:sd')),
])
...
def getCpuDictionary():
    cpuData = OrderedDict()
    if rpi_cpu_tuple != '':
        cpuData[RPI_CPUP] = '{:.3f}'.format(rpi_cpu_tuple[0])
        cpuData[RPI_CPUN] = '{:.0f}'.format(rpi_cpu_tuple[1])
    return cpuData
